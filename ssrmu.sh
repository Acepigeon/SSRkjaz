...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/Alextraszaa/SSRkjaz/master/bbr.sh; then
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: ceshi):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="ceshi"
...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/Alextraszaa/shadowsocksr/archive/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/Alextraszaa/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Alextraszaa/SSRkjaz/master/service/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Alextraszaa/SSRkjaz/master/service/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
Check_Libsodium_ver(){
	...
	Libsodiumr_ver=$(wget -qO- "https://github.com/Alextraszaa/libsodium/tags"|grep "/Redundant2333/libsodium/releases/tag/"|head -1|sed -r 's/.*tag\/(.+)\">.*/\1/')
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		wget  --no-check-certificate -N "https://github.com/Alextraszaa/libsodium/releases/download/${Libsodiumr_ver}/libsodium-${Libsodiumr_ver}.tar.gz"
		echo -e "${Info} 解压..."
...
Install_ServerSpeeder(){
	...
	#借用91yun.rog的开心版锐速
	wget --no-check-certificate -qO /tmp/serverspeeder.sh https://raw.githubusercontent.com/Alextraszaa/serverspeeder/master/serverspeeder.sh
...
Install_LotServer(){
	...
	#Github: https://github.com/Alextraszaa/serverSpeeder_Install
	wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/Alextraszaa/serverSpeeder_Install/master/appex.sh"
...
Uninstall_LotServer(){
	...
	[[ -z ${unyn} ]] && echo && echo "已取消..." && exit 1
	if [[ ${unyn} == [Yy] ]]; then
		wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/Alextraszaa/serverSpeeder_Install/master/appex.sh" && bash /tmp/appex.sh 'uninstall'
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/Alextraszaa/SSRkjaz/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/Alextraszaa/SSRkjaz/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/Alextraszaa/SSRkjaz/master/ssrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/Alextraszaa/SSRkjaz/master/ssrmu.sh" && chmod +x ssrmu.sh
